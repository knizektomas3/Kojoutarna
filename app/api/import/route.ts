...
export async function POST(request: Request) {
  ...
  const { type, generationId, csv } = await request.json()
  ...
  if (type === 'produkce') {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const { error } = await supabase.from('productions').upsert(
        { user_id: user.id, generation_id: generationId, date, egg_count },
    ...
    for (let i = 0; i < dataRows.length; i++) {
      const [date, income_type, amountStr, customer_name, customer_type] = dataRows[i]
    ...
    for (let i = 0; i < dataRows.length; i++) {
      const [date, category, acquisitionSub, operationalSub, amountStr] = dataRows[i]
      if (!date || !amountStr) continue
      const amount = parseInt(amountStr)
      if (isNaN(amount)) { errors.push(`Řádek ${i + 2}: neplatná částka "${amountStr}"`); continue }
