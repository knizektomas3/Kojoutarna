...
function makeSvg(size) {
  ...
  // Vajíčko pomocí 4 symetrických kubických bezierů
  // Nahoře špičatější, dole kulatější, nejširší mírně pod středem
  const path = [
    `M ${s*0.500},${s*0.130}`,
  ...
  // Odlesk – mírně nakloněná elipsa v levé horní části vajíčka
  const hlCx = s * 0.385
  const hlCy = s * 0.295
  const hlRx = s * 0.072
  const hlRy = s * 0.110
